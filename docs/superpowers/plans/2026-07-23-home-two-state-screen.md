...
| D8 (=ТЗ 12.4) | Морф кнопки мікрофон↔стрілка в доці чи дві окремі кнопки? | ~~Морф, кросфейд іконок 140ms~~ — **переглянуто:** іконка в доці завжди стрілка «надіслати», незалежно від стану поля чи екрана. Логіка кліку (порожнє поле → `onMicTap`, є текст → `onSubmitText`) не змінилась, змінилась тільки іконка. | Task 5, переглянуто в Task 9 |
